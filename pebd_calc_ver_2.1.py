...
class PayDateCalculator:
    def __init__(self, root):
        ...
        tk.Label(root, text="Service Periods (Active Duty):").grid(row=5+len(self.dep_entries), column=0, columnspan=6, pady=5)
        ...
        tk.Label(root, text="Inactive Periods (Pre-converted to Active Duty Equivalent):").grid(row=9+len(self.dep_entries)+len(self.active_entries), column=0, columnspan=6, pady=5)
        tk.Label(root, text="Start").grid(row=10+len(self.dep_entries)+len(self.active_entries), column=0, padx=5, pady=2)
        tk.Label(root, text="End").grid(row=10+len(self.dep_entries)+len(self.active_entries), column=1, padx=5, pady=2)
        self.add_inactive_period(row=11+len(self.dep_entries)+len(self.active_entries))
        tk.Button(root, text="Add Inactive Period", command=lambda: self.add_inactive_period(row=11+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries))).grid(row=12+len(self.dep_entries)+len(self.active_entries), column=0, columnspan=6, pady=5)
        
        tk.Label(root, text="Lost Time Periods (Subtracted from Creditable Service):").grid(row=13+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries), column=0, columnspan=6, pady=5)
        tk.Label(root, text="Start").grid(row=14+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries), column=0, padx=5, pady=2)
        tk.Label(root, text="End").grid(row=14+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries), column=1, padx=5, pady=2)
        self.add_lost_period(row=15+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries))
        tk.Button(root, text="Add Lost Time Period", command=lambda: self.add_lost_period(row=15+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries)+len(self.lost_entries))).grid(row=16+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries), column=0, columnspan=6, pady=5)
        
        tk.Label(root, text="Constructive Service Years:").grid(row=17+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries)+len(self.lost_entries), column=0, padx=5, pady=5)
        self.entry_constructive = tk.Entry(root)
        self.entry_constructive.grid(row=17+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries)+len(self.lost_entries), column=1, padx=5, pady=5)
        
        tk.Button(root, text="Calculate", command=self.calculate).grid(row=18+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries)+len(self.lost_entries), column=0, columnspan=3, pady=10)
        tk.Button(root, text="Reset", command=self.reset).grid(row=18+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries)+len(self.lost_entries), column=3, columnspan=3, pady=10)
        
        self.result_text = scrolledtext.ScrolledText(root, width=100, height=20, wrap=tk.WORD, font=("Courier", 10))
        self.result_text.grid(row=19+len(self.dep_entries)+len(self.active_entries)+len(self.inactive_entries)+len(self.lost_entries), column=0, columnspan=6, padx=5, pady=5)
    ...
    def reset(self):
        ...
        self.add_inactive_period(row=11+len(self.active_entries))
        
        for start_entry, end_entry in self.lost_entries:
            start_entry.destroy()
            end_entry.destroy()
        self.lost_entries.clear()
        self.add_lost_period(row=15+len(self.active_entries)+len(self.inactive_entries))
    ...
    def calculate(self):
        try:
            ...
            # Conditional PEBD calculation based on Reenlistment Date vs EOS
            if reenlist_dt < eos_dt:
                # Second case logic: No adjustment, exact subtraction
                pebd = subtract_days_from_date(reenlist_date, net_service_days)
            else:
                # First case logic: Add 1 day to subtraction for historical EOS
                pebd = subtract_days_from_date(reenlist_date, net_service_days + 1)
            ...
            result_text_content += (
                f"\n\nNote: {offset_note}\n"
                f"Note: Inactive periods must be pre-converted to active duty equivalent days (e.g., 1 day per drill) per DoDFMR Vol 7A, Ch 1."
            )
